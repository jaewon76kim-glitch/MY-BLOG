...
//
// normalization SNR x = γ/γ̄ 표본을 고정폭 빈(bin)에 누적한다. 아웃티지 실측 확률은 원표본 리스트를
// 따로 저장하지 않고, 누적된 히스토그램 빈으로부터 "임계값 미만 누적 비율"을 즉시 계산한다.
// 이렇게 하면 아웃티지 임계값 γ_th 슬라이더를 움직여도(분포 자체는 안 변하므로) 통계를 리셋할
// 필요가 없다 — 모델/parameter(모델 종류, K, m) 변경 시에만 reset()을 호출한다.

var Stats = (function () {
  'use strict';

  var NBINS = 100;
  var XMAX = 10; // normalization SNR x 커버 범위 (0~10). 이 이상은 오버플로 빈에 누적.
  ...
  // 히스토그램 밀도(normalization된 확률밀도): Σ density*BIN_WIDTH ≈ 1 (오버플로 빈 제외)
