// linkbudget.js - link budget 핵심 계산 모듈 (전역 변수 방식, type="module" 사용 안 함)
//
// 근거: 위성통신_소스.tex
//   - \section{자유공간 path loss와 프리스 공식} (148절)      → 자유공간 path loss dB식
//   - \section{link budget, 수신감도, 아웃티지 확률} (231절)    → Es/N0, 수신감도, 아웃티지(Q함수)
//   - \section{위성 link budget의 완성: EIRP와 G/T} (905절)     → EIRP, Tsys, G/T, 업/다운링크 합산식

var LinkBudget = (function () {
  'use strict';

  // ---- 물리 상수 ----
  var T0 = 290;            // noise figure 기준온도 [K] (tex 905절: T_e = T0(F-1))
  var KB = 1.380649e-23;   // 볼츠만 상수 [J/K] (CODATA)

  // 볼츠만 상수를 dB로: 10*log10(kB) ≈ -228.6 dBW/K/Hz
  // (link budget에서 흔히 쓰이는 "-228.6 dBW/K/Hz" 상수. 항상 수식으로 계산해
  //  하드코딩 오탈자를 방지한다.)
  ...
  // ---- 2.2 G/T (tex 905절) ----
  // Te = T0*(F-1) [F는 선형 noise figure], Tsys = Tant + Te, (G/T)_dB = GR_dB - 10log10(Tsys)
  ...
  // ---- orbit 기하: 슬랜트 거리 (apps/doppler-sim/js/sim.js의 slantRange와 동일한
  // 구면 기하 공식 사용 — 지구반경 Re, altitude h, elevation angle El) ----
  ...
  // ---- 2.3 자유공간 path loss (tex 148절 dB 실무식, 231절에서 재인용) ----
  ...
  // ---- 2.5 수신감도 / 최대허용 path loss (보조 지표) ----
  // tex 231절(272~292행): 수신감도는 요구 (Es/N0)_min을 만족하는 "문턱 수신전력"이며
  // 실제 path loss와 무관하게 Tsys, Rs, (Es/N0)_min만으로 정해진다.
  //   N0_dBW/Hz = kB_dB + 10log10(Tsys)
  //   S_rx,dBW = (Es/N0)_min,dB + Rs_dB + N0_dBW/Hz
  // 최대허용 path loss는 spec 2.5의 "S_rx,dBm = Pt+GT+GR-Lp" 관계를, 위 문턱값과
