/* bottleneck-calc.js — 섹션 4: 1x1 convolution 연산량 절감 계산기
 * "직접 큰 kernel 적용" vs "1x1 bottleneck 후 큰 kernel 적용"의 곱셈 연산 횟수를 비교한다.
 *
 * GoogLeNet 예시 기본값(교재 8부 원문 그대로 재현):
 *   H=W=11(feature map 공간 크기), C_in=120, k=5(5x5 kernel), C_out=20(5x5 kernel 20개),
 *   C_bottleneck=6(1x1 kernel 6개로 channel을 120→6으로 줄임)
 *   직접 적용: 11×11×5×5×120×20 = 7,260,000 ≈ 726만 회
 *   bottleneck 후 적용: 11×11×1×1×120×6 + 11×11×5×5×6×20 = 87,120 + 363,000 = 450,120 ≈ 45만 회
 * (원문에서 1x1은 channel을 120→6으로 줄이며, 5x5 kernel 20개가 최종 출력 channel 수 C_out=20을 만든다.
 *  "120→20"이 아니라 "120→6"이 bottleneck channel 수이므로 이 값을 기본값으로 사용해야 726만/45만이 정확히 재현된다.)
 */
...
function bnRenderText(calc) {
  ...
  document.getElementById('bn-ratio-text').textContent =
    'bottleneck 사용 시 연산량이 약 ' + ratio.toFixed(1) + '배 절감됩니다 (직접 적용 대비 ' +
    (100 / ratio).toFixed(1) + '%).';

  document.getElementById('bn-breakdown-text').innerHTML =
    '1×1 bottleneck 단계: ' + bnState.H + '×' + bnState.W + '×1×1×' + bnState.Cin + '×' + bnState.Cbottleneck +
    ' = ' + calc.bottleneck.stage1.toLocaleString('ko-KR') + '회<br>' +
    bnState.k + '×' + bnState.k + ' 큰 kernel 단계: ' + bnState.H + '×' + bnState.W + '×' + bnState.k + '×' + bnState.k +
...
function bnRenderChart(calc) {
  ...
  var data = {
    labels: ['직접 ' + bnState.k + '×' + bnState.k + ' 적용', '1×1 bottleneck 후 적용'],
    datasets: [{
      label: '곱셈 연산 횟수',
