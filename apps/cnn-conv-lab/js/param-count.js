/* param-count.js — 섹션 3: 완전연결 vs convolution parameter 수 비교
 * 입력 크기·channel 수·kernel 크기·출력 channel 수를 받아 두 방식의 parameter 개수를 계산하고
 * Chart.js 막대그래프(로그 스케일)로 비교한다.
 *
 * 완전연결 비교 기준(가정): "출력 feature map의 각 유닛이 입력 전체와 독립적으로 완전연결된 경우".
 * 즉 입력 유닛 수(H×W×C_in)를, 이 convolution층이 만들어내는 출력 feature map 전체 유닛 수
 * (Hout×Wout×C_out, padding 없음·stride 1 가정)에 전부 연결했다고 가정한다.
 * LeNet-5 예시(입력 32×32×1, 5×5 kernel 6개)에서는 Hout=Wout=28이므로 출력 유닛 수 4704개,
...
function pcRenderText(calc) {
  ...
  if (elAssump) {
    elAssump.textContent =
      '출력 feature map 크기: ' + calc.Hout + '×' + calc.Wout + '×' + pcState.Cout + ' = ' +
...
function pcRenderChart(calc) {
  ...
  var data = {
    labels: ['완전연결(FC)', 'convolution(부분연결+weight sharing)'],
    datasets: [{
      label: 'parameter 개수',
  ...
  var options = {
    ...
    scales: {
      y: {
        type: 'logarithmic',
        title: { display: true, text: 'parameter 개수 (로그 스케일)', color: '#a0a0c0' },
        ticks: { color: '#a0a0c0' },
