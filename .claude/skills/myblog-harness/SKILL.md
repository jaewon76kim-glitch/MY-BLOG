...
### 3.3 카테고리 (고정 순서, `js/home.js`의 `CATEGORIES`와 반드시 일치, 2026-07-31 갱신)

```
위성통신, 3GPP표준, 머신러닝과_인공지능, 반도체와_AI산업, OS, 해석역학과_장이론,
...
- **예외 — `반도체와_AI산업`**: doc 폴더에 대응하는 하위 폴더가 없는 독립
  카테고리(2026-07-29 신설). 실제 기업의 기술적 관계·산업 동향을 분석하는 글
  (예: "AI 생태계 지도", "온디바이스 AI 지도")을 담는다. doc 폴더 소재를 먼저
  정하는 절차 없이 WebSearch로 최신 동향을 조사해 바로 쓸 수 있다.

### 3.7 프라이버시 경계 (2026-07-30 신설, 2026-07-31 폴더 확장, 반드시 지킬 것)

이 저장소는 GitHub public repo다. **`doc/유진`(아들의 대입 전략 자료 —
실명·학교·성적), `doc/육상`(미성년 육상 선수 6명의 실명·기록), `doc/이력`
(본인 실명 경력 이력서) 폴더의 내용은 어떤 경우에도 이 블로그의 소재로
쓰지 않는다.** 상위 오케스트레이터(`C:\Users\hugok\Claude\Projects\orchestrator
\CLAUDE.md`)가 관리하는 프로젝트 레지스트리에서 이 저장소는 "공개"로 표시되어
있고, 위 세 폴더는 "비공개"다.

- doc에서 소재를 가져올 때는 항상 어느 하위 폴더인지 확인하고, "비공개"
  표시가 붙은 폴더는 소재 후보에서 제외한다.
- 예외: doc/유진에서 쓰인 **프레임워크(사고 도구)** 자체(예: "AI 생태계
  6단계" 같은 일반적 분석틀)는 학생 개인정보를 포함하지 않으므로 재사용 가능.
  안 되는 건 그 틀에 채워진 구체적 개인 데이터다.
...
0. **`posts/index.json` 스키마 검증 — `scripts/validate_posts.py`** (2026-07-30 추가)
   `python3 -m json.tool`은 JSON 문법만 보고 필드명·카테고리 유효성은 안 본다는
   빈틈이 있었음. 이 스크립트는 필수 필드(`slug/title/date/tags/category/
   description/wordCount`) 존재, `category`가 `js/home.js`의 `CATEGORIES`에
   실제로 있는지(카테고리 목록은 여기서 직접 파싱해 단일 출처 유지), `date`
   유효성(datetime 파싱), slug 중복까지 확인한다. post-article 스킬의 Review
   단계(§3.6)에서 매번 실행한다.
